    ...
    if(/\S+@\S+\.\S+/.test(this.state.email) == false){
      ...
      api.callApi('changeEmailRequest', 'POST', userData, response => {
          ...
          if(response['responseCode'] == 400){
             ...
             this.successMessage(
               'Er is een email met een veiligheids code verzonden naar: ' + response['oldEmail'],
              );
      ...
      <View style={styles.ChangeEmailReqcontainer}>
        <View style={styles.ChangeEmailReqcard} elevation={5}>
          <Text style={{margin: 15, fontWeight: 'bold', fontSize: 14, color: 'white'}}>
		        Wil je je e-mail adres veranderen? Vul hieronder je huidige email adres in en ontvang een veiligheidscode om je e-mail adres te vervangen!
